---
# ──────────────────────────────────────────────────────────────────────────
# DESIGN.md — Ollama marketing-surface design system
# Format: @google/design.md   ·   Lint: npx @google/design.md lint DESIGN.md
#
# Every group.token reference in the prose (for example colors.primary) resolves
# to an entry here. Group names match the reference prefix exactly:
#   colors  -  typography  -  spacing  -  rounded  -  component
# ──────────────────────────────────────────────────────────────────────────

meta:
  name: Ollama Marketing
  sources: ["/", "/pricing"]
  base-unit: 8px
  philosophy: >
    The system is the documentation, and the documentation is the system.
    Paper-white canvas end-to-end, pill geometry for everything interactive,
    a single inverted dark surface as the only "look here" cue.

# ── Colors ──────────────────────────────────────────────────────────────
colors:
  # Brand & Accent
  primary: "#000000"          # the brand: every CTA, black pill, nav link, solid icon
  ink-deep: "#090909"         # pressed-state black for the primary pill

  # Surface
  canvas: "#ffffff"           # the page itself; nearly every surface
  surface-soft: "#fafafa"     # install pill, search pill, secondary chips
  surface-dark: "#171717"     # the single inverted surface (Max card, dark CTA)
  hairline: "#e5e5e5"         # 1px card border, dividers
  hairline-strong: "#d4d4d4"  # rare stronger divider

  # Text
  ink: "#000000"              # headlines, nav links, button text on light, prices
  charcoal: "#525252"         # list-item text, disabled secondary copy
  body: "#737373"             # default paragraph copy, FAQ answers, footer links
  mute: "#a3a3a3"             # captions, terminal comment gray, lowest-emphasis
  on-dark: "#ffffff"          # primary text on surface-dark
  on-dark-mute: "rgba(255,255,255,0.7)"  # secondary copy in the dark Max card
  on-primary: "#ffffff"       # text on the black primary pill

  # Semantic — terminal traffic lights only (no error/success/warning system)
  terminal-red: "#ff5f56"     # close-window dot
  terminal-yellow: "#ffbd2e"  # minimize dot
  terminal-green: "#27c93f"   # zoom dot

  # Focus
  focus-ring: "rgba(59,130,246,0.5)"  # browser-default translucent blue ring

# ── Typography ──────────────────────────────────────────────────────────
# Fonts: SF Pro Rounded (display) · ui-sans-serif (body) · ui-monospace (code)
typography:
  display-xl:     { size: 36px, weight: 500, line-height: 1.11, letter-spacing: 0, font: "SF Pro Rounded" }
  display-lg:     { size: 30px, weight: 500, line-height: 1.2,  letter-spacing: 0, font: "SF Pro Rounded" }
  heading-lg:     { size: 24px, weight: 600, line-height: 1.33, letter-spacing: 0, font: "SF Pro Rounded" }
  heading-md:     { size: 20px, weight: 500, line-height: 1.4,  letter-spacing: 0, font: "SF Pro Rounded" }
  heading-sm:     { size: 18px, weight: 500, line-height: 1.56, letter-spacing: 0, font: "SF Pro Rounded" }
  body-md:        { size: 16px, weight: 400, line-height: 1.5,  letter-spacing: 0, font: "ui-sans-serif" }
  body-strong:    { size: 16px, weight: 500, line-height: 1.5,  letter-spacing: 0, font: "ui-sans-serif" }
  body-sm:        { size: 14px, weight: 400, line-height: 1.43, letter-spacing: 0, font: "ui-sans-serif" }
  body-sm-strong: { size: 14px, weight: 500, line-height: 1.43, letter-spacing: 0, font: "ui-sans-serif" }
  caption-sm:     { size: 12px, weight: 400, line-height: 1.33, letter-spacing: 0, font: "ui-sans-serif" }
  code-md:        { size: 16px, weight: 400, line-height: 1.5,  letter-spacing: 0, font: "ui-monospace" }
  code-sm:        { size: 14px, weight: 400, line-height: 1.43, letter-spacing: 0, font: "ui-monospace" }
  button-md:      { size: 14px, weight: 500, line-height: 1,    letter-spacing: 0, font: "ui-sans-serif" }
...
component:
  ...
  terminal-card:
    ...
    padding: "{spacing.lg}"
    radius: "{rounded.lg}"
    body-type: "{typography.code-sm}"
    comment-color: "{colors.mute}"
    command-color: "{colors.ink}"
  terminal-traffic-lights:
    size: 12px
    radius: "{rounded.full}"
    gap: "{spacing.xs}"
    dots: ["{colors.terminal-red}", "{colors.terminal-yellow}", "{colors.terminal-green}"]
  pricing-card:
    background: "{colors.canvas}"
    border: "1px solid {colors.hairline}"
    padding: "{spacing.xxl}"
    radius: "{rounded.lg}"
    tier-name-type: "{typography.heading-md}"
    price-type: "{typography.display-lg}"
    cta: "{component.button-primary}"
    list-header-type: "{typography.body-sm-strong}"
  pricing-card-dark:
    extends: pricing-card
    background: "{colors.surface-dark}"
    text: "{colors.on-dark}"
    secondary-text: "{colors.on-dark-mute}"
    cta: "{component.button-pill-on-dark}"
  feature-bullet:
    checkmark-color: "{colors.ink}"
  ...
  faq-row:
    ...
    question-type: "{typography.heading-sm}"
    question-color: "{colors.ink}"
    answer-type: "{typography.body-md}"
    answer-color: "{colors.body}"
    gap: "{spacing.xs}"
  cta-strip-dark:
    background: "{colors.surface-dark}"
    text: "{colors.on-dark}"
    type: "{typography.heading-lg}"
    padding: "24px 32px"
    radius: "{rounded.lg}"
  ...
  primary-nav:
    ...
    text: "{colors.ink}"
    height: 56px
    type: "{typography.body-sm-strong}"
    radius: "{rounded.none}"
...
- Identical layout to `{component.pricing-card}` but with `{colors.surface-dark}` background, `{colors.on-dark}` text, `{colors.on-dark-mute}` secondary text, and `{component.button-pill-on-dark}` CTA. The inversion is the system's single "look here" cue.
