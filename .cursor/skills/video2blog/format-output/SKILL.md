...
- `memory/CONFIG.md` 约定目录
- `output/README.md` 归档结构与命名约定

## 落盘强契约（PASS / DRAFT 共通）

| 文件 | 路径 | 何时写 |
|---|---|---|
| 定稿正文 | `output/Posts/<YYYY>/<YYYY-MM-DD>-<slug>.md` | PASS |
| 草稿正文 | `output/Posts/<YYYY>/DRAFT-<YYYY-MM-DD>-<slug>.md` | REVIEW 且用户接受先落 |
| 评分报告 | `output/Reviews/<YYYY-MM-DD>-<slug>.review.md` | **PASS / REVIEW 都写**（含 Step 7 评分表 + Re-Brief 全文） |
| HISTORY 增行 | `memory/HISTORY.md` | PASS / DRAFT 都写（仅 10 条） |
...
- **PASS 与 DRAFT 都须**在 `memory/HISTORY.md` 表格追加一行：
...
1. 生成 slug，确认年份目录 `output/Posts/<YYYY>/` 存在；不存在则创建。
