...
_KNOWLEDGE_GROUPS: list[tuple[str, bool, list[tuple[str, str, str, bool]]]] = [
    (
        "写作偏好",
        False,
        [("memory/PREFERENCES.md", "写作偏好", "人称/语言/受众/目标字数/禁用套话/格式", False)],
    ),
    (
        "风格与范文",
        False,
        [("knowledge/STYLE_GUIDE.md", "风格指南", "18 条写作硬规则，优先级高于范文", False)],
    ),
    (
        "运行合同 · 路由",
        True,
        [
            (
                "WORKFLOW.md",
                "运行合同",
                "模式/各 Step 合同/路由人设；改错影响整条流水线，≤70 行",
                True,
            )
        ],
    ),
    (
        ...
        [
            (
                ".cursor/skills/video2blog/clean-transcript/SKILL.md",
                "Step 3 清洗",
                "清洗 ASR 转录稿",
                False,
            ),
            (
                ".cursor/skills/video2blog/extract-insights/SKILL.md",
                "Step 4 提炼",
                "提炼核心观点",
                False,
            ),
            (
                ".cursor/skills/video2blog/structure-narrative/SKILL.md",
                "Step 5 骨架",
                "搭建博文骨架",
                False,
            ),
            (
                ".cursor/skills/video2blog/rewrite-blog/SKILL.md",
                "Step 6 改写",
                "第一人称撰写博文",
                False,
            ),
            (
                ".cursor/skills/video2blog/quality-check/SKILL.md",
                "Step 7 质检",
                "六维评分；输出格式被代码解析，改错会断功能",
                True,
            ),
            (
                ".cursor/skills/video2blog/format-output/SKILL.md",
                "Step 8 落盘",
                "frontmatter 被代码校验，改错会断功能",
                True,
            ),
        ],
    ),
...
def register(app: FastAPI, service: EngineJobService, root: Path) -> None:
    ...
    def write_knowledge_file(payload: KnowledgeFileRequest) -> dict[str, Any]:
        ...
        """
        from video2blog.utils import PLACEHOLDER_RE, atomic_write
        ...
        if rel == "WORKFLOW.md" and len(content.splitlines()) > 70:
            errors.append(
                f"WORKFLOW.md 超过 70 行（当前 {len(content.splitlines())} 行），引擎校验会拒绝"
            )
    ...
    def list_examples() -> list[dict[str, Any]]:
        ...
        if not ex_dir.is_dir():
            return []
        items = [
            _example_meta(f) for f in sorted(ex_dir.glob("*.md")) if f.name.lower() != "readme.md"
        ]
        # 最新上传的排前面
